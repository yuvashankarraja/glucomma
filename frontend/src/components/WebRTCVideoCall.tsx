...
export default function WebRTCVideoCall({ roomId, role, onClose }: WebRTCVideoCallProps) {
  ...
  // Independent video stream states to render placeholders instead of blocking connections
  ...
  const startConsultation = async () => {
    try {
      ...
      socketRef.current.onopen = () => {
        ...
        // Notify other participants that we have joined the room
  ...
  const handleSignalingMessage = async (data: any) => {
    ...
    try {
      if (data.type === 'ready') {
        // The other participant joined: if we are the patient, initiate the WebRTC offer
        if (role === 'patient') {
          createCallOffer();
        }
      }
        ...
        <div className="flex-1 bg-gray-950 p-6 flex flex-col justify-between relative">
            ...
            <div className="relative bg-gray-900 rounded-2xl overflow-hidden aspect-video border border-white/5 flex items-center justify-center">
              {!remoteVideoActive ? (
                <div className="text-center p-4">
                  ...
                  <p className="text-[10px] text-gray-500 mt-1">Waiting for participant video...</p>
                </div>
              ) : (
                <video 
                  ref={remoteVideoRef} 
          ...
          <div className="flex items-center justify-center gap-3 z-10 bg-gray-900/60 backdrop-blur-md p-4 rounded-2xl border border-white/5 w-fit mx-auto">
            ...
            <button
              ...
              } ${!localVideoActive ? 'opacity-40 cursor-not-allowed' : ''}`}
            >
